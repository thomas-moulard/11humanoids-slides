...
\begin{frame}{Closed-loop trajectory control scheme}
  ...
  \begin{center}
    ...
    \begin{tikzpicture}[>=latex]
      ...
      \draw<3->[->] (loc.west) -- (corr.east) node [at start,left,font=\footnotesize,yshift=0.2cm] {$\hat{\mathbf{x}}_t$};
      \draw<2->[->] (gen.south) -- (corr.north) node [at start,left,font=\footnotesize,yshift=-0.2cm]
           {$\gamma_{\text{left~foot}}(t), \gamma_{\text{right~foot}}(t), \gamma_{\text{com}}(t), \mathbf{x}(t)$};
      \draw<3->[->] (corr.south) -- (ctrl.north) node [at start,left,font=\footnotesize,yshift=-0.25cm]
           {$\gamma^{(n)}_{\text{left~foot}}(t), \gamma^{(n)}_{\text{right~foot}}(t), \gamma^{(n)}_{\text{com}}(t)$};

      \draw<4->[->] (ctrl.south) --++ (0,-1) node [at start,left,font=\footnotesize,yshift=-0.25cm]
           {$\dot{\mathbf{q}}_t$};
  ...
  \begin{center}
    ...
    \begin{tikzpicture}
      ...
      \draw<7-> [
        ...
        font=\tiny,
        decorate
      ] (axis cs:2.1,0.9) -- (axis cs:4.1,0.9)
      ...
      \draw<8-> [
        ...
        font=\tiny,
        decorate
      ] (axis cs:5.1,0.9) -- (axis cs:7.1,0.9)
      ...
      \draw<9-> [
        ...
        font=\tiny,
        decorate
      ] (axis cs:2.1,4.25) -- (axis cs:5.1,4.25)
      node [pos=0.5,anchor=north,yshift=-0.55cm] {CoM corr.};
      ...
      \draw<4->[style=solid,color=red,line width=1.25] (\dx, \dy - 0.1 + 0.5)
      rectangle (\dx + \w, \dy + \h - 0.1 + 0.5);

      % right step planned 1
      \filldraw[pattern=north east lines] (\dx + 1., \dy + 1. + 0.5)
      rectangle (\dx + 1. + \w, \dy + \h + 1. + 0.5)
      node [pos=0.5,anchor=north,font=\tiny,yshift=0.75cm] {step 1};

      % right step planned 1 - corrected
      \draw<5->[style=solid,color=green,line width=1.25] (\dx + 1., \dy + 1. + 0.5)
      rectangle (\dx + 1. + \w, \dy + \h + 1. + 0.5);
